...
clear all; close all;
L       = 100; % orginal L =100；
S       = 32;  J       = 12;b       = 96;M       = 32;K_a     = 300;iter    = 10;
% Specify fading
fading.type         = 'no_fading';
fading.lower_limit  = 0; fading.upper_limit  = 0;

% Data_profile speficies the number of data bits per sections
data_profile     = ones(S,1)*(3);
data_profile(1)  = J;data_profile(32) = 0;data_profile(31) = 0;data_profile(30) = 0;

assert(abs(sum(data_profile)-b)<eps);
rate = sum(data_profile)/(S*L);

Eb_N0_dB    = -5:1:5; % -5:1:5
